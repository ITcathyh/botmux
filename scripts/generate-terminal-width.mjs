...
//   (b) Node \p{Emoji_Presentation}——覆盖 Unicode 14/15/16 新增 emoji(🫠🩷🫨…),
//       这些在 xterm-11 表里还是 1,但现代本地/SSH 终端按 2 画,只锁旧 oracle 会欠计。
...
function extractRanges() {
  ...
  const widthOf = cp => {
    ...
    return EMOJI_PRESENTATION.test(String.fromCodePoint(cp)) ? 2 : 1;
  };
