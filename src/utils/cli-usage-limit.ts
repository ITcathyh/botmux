 ...
 * CLIs (codex / grok / traex / pi) also set `reliableTurnTerminal` but emit NO
 * structured `limited` state, so suppressing their screen `rate` would drop the
 * Dashboard「需要你」signal + backoff on a real 429. Extracted as a pure
 * predicate so the split has a direct unit-test surface (see
 * cli-usage-limit.test.ts) — a future adapter can't silently re-broaden it.
 */
