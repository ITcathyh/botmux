...
async function startInitialPassthroughSession(args: {
  ...
  const {
    ...
    parsed, commandContent, senderOpenId, senderUnionId, memberUnionId, ownerOpenId, ownerUnionId, creatorOpenId, botSender,
  } = args;
...
async function handleThreadReply(data: any, ctx: RoutingContext): Promise<void> {
  ...
  if (!await enforceMessageQuotaForCliInput(larkAppId, ctxChatId ?? data?.message?.chat_id, quotaSenderOpenId, parsed.messageId, anchor, threadTeamTrustUnionId, threadSenderUnionId, ctxChatType, isBotSenderType || isForeignBot)) {
    return;
  }
