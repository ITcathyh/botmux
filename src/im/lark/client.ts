...
import { isMobileEntry, normalizeMobileEntry } from '../../setup/bot-config-editor.js';
    ...
    if (mobiles.length > 0) {
      ...
      try {
        ...
        if (res.code !== 0) {
          ...
          const userList: any[] = res.data?.user_list ?? [];
          // Build normalized(mobile) → user_id, then backfill each raw entry.
          const byNorm = new Map<string, string>();
          for (const item of userList) {
            if (item.user_id && item.mobile) byNorm.set(normalizeMobileEntry(String(item.mobile)), item.user_id);
            else if (!item.user_id) logger.warn(`Could not resolve mobile: ${item.mobile}`);
          }
          for (const norm of mobiles) {
            const rawEntry = mobileRawByNorm.get(norm) ?? norm;
            // API may echo the mobile with/without country code; try the exact
            // normalized form first, then a +86-stripped / -added CN variant.
            const uid = byNorm.get(norm)
              ?? byNorm.get(norm.replace(/^\+86/, ''))
              ?? (norm.startsWith('1') ? byNorm.get('+86' + norm) : undefined);
            if (uid) {
