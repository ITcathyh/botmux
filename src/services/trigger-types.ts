...
export interface TriggerResponse {
  ...
  /** Token usage for the completed turn's session (best-effort, parsed from the
   *  CLI transcript). Present on `state:'completed'` when the transcript is
   *  readable; omitted otherwise. Field names mirror the caller's TaskTokenUsage. */
  usage?: {
    inputTokens: number;
    outputTokens: number;
